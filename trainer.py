    ...
    for images, labels in testloader:
        for i in range(len(labels)):
            ...
            with torch.no_grad():  # no gradients are computed during forward pass as training isn't being done
                logProbs = evalModel(
                    img
                )  # stores log probabilities predicted by the model for current image
            ...
            predictedLabel = probabilityList.index(
                max(probabilityList)
            )  # finds index of the most probable label
...
def saveModel(saveModel):
    print("Saving model...")
    if not os.path.exists("./model"):
        os.makedirs("./model")

    torch.save(saveModel.module.state_dict(), "./model/mnistModel.pt")


# Setting up MNIST dataset
transform = transforms.Compose(
    [
        transforms.ToTensor(),  # separates image into rgb channels and converts each pixel to the brightness of their colour between 0-255 which are then scaled to 0-1
        transforms.Normalize(
            (0.5,), (0.5,)
        ),  # normalises the tensor so it is between -1 and 1
    ]
)
...
#Train if untrained, otherwise load pretrained model
if os.path.exists("./model/mnistModel.pt"):
    model.load_state_dict(torch.load("./model/mnistModel.pt"))
    model.eval()  # changes model to evaluation mode
    print("Model already trained, skipping training")
    modelPretrained = True
else:
    trainModel()
    model = model
